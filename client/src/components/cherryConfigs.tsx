...
const customMenu_question = Cherry.createMenuHook('帮助中心', {
  ...
  subMenuConfig: [
    { noIcon: true, name: '快捷键', onclick: () => { cherryMarkdown.toolbar.menus.hooks.customMenuCName.fire(null, 'shortKey') } },
    { noIcon: true, name: '联系我们', onclick: () => { cherryMarkdown.toolbar.menus.hooks.customMenuCName.fire(null, 'github') } },
    { noIcon: true, name: '更新日志', onclick: () => { cherryMarkdown.toolbar.menus.hooks.customMenuCName.fire(null, 'release') } },
  ]
});
...
export const defaultConfig = {
  ...
  previewer: {
    ...
    lazyLoadImg: {
      ...
      beforeLoadOneImgCallback: (img: HTMLImageElement) => {
        return true;
      },
      // 加载一张图片失败之后的回调函数
